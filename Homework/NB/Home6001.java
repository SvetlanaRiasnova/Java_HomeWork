...
public class Home6001 {
    public static void main(String[] args) {
        Notebook nb1 = new Notebook("Asus", "15 D543MA-DM1368", "i5", 4, 1000, "Windows 10", "черный");
        Notebook nb2 = new Notebook("Acer", "Aspire 3 A315-23-R0HR", "i7", 4, 256, "Windows 10", "черный");
        Notebook nb3 = new Notebook("Lenovo", "IdeaPad 3 15IML05", "AMD", 8, 1000, "Windows 11", "серый");
        Notebook nb4 = new Notebook("MSI", "Modern 15 B11-002RU", "i5", 8, 256, "Windows 11", "черный");
        Notebook nb5 = new Notebook("Asus", "VivoBook Pro 14 K3400PA-KP109", "i7", 16, 512, "Windows 10", "серебристый");
        Notebook nb6 = new Notebook("MSI", "Summit E13 Flip Evo A12MT-061RU", "i5", 16, 512, "Без ОС", "серый");
        Notebook nb7 = new Notebook("Huawei", "MateBook D16 RLEF-X", "i7", 16, 512, "Windows 10", "серый");
        Notebook nb8 = new Notebook("Asus", "ROG Zephyrus G14 GA402RJ-L4045", "AMD", 8, 1000, "Без ОС", "серый");
        Notebook nb9 = new Notebook("MSI", "Raider GE78 HX 13VH-094RU", "AMD", 32, 2000, "Windows 11", "черный");
        Notebook nb10 = new Notebook("Apple", "MacBook Air", "i7", 8, 256, "macOS", "серый");
        ...
        System.out.println();
        System.out.println("Для поиска подходящей модели, введите цифру, соответствующую необходимому критерию: \n");
        boolean filter = true;
        while (filter) {
            ...
            switch (operation) {
                ...
                case "0":
                    ...
                    break;
                default:
                    System.out.println("Нет подходяего параметра");
                    System.out.println();
                    break;
            }
        }
    ...
    public static void filterHardDisk(HashSet<Notebook> notebooks, Scanner scanner) {
        ...
        System.out.println();
        System.out.println("В нашем каталоге представлены ноутбуки с объемом SSD "
    ...
    public static void filterOS(HashSet<Notebook> notebooks, Scanner scanner) {
        ...
        if (operSystems.contains(oper)) {
            ...
            System.out.println("Нет такой ОС. Выберите другую.");
    ...
    // поиск по процессору
    public static void filterProcc(HashSet<Notebook> notebooks, Scanner scanner) {
        TreeSet<String> pr = new TreeSet<>();
        for (Notebook note : notebooks) {
            pr.add(note.getProcc());
        }
        System.out.println();
        System.out.println("Доступные процессоры: "
                + pr.toString().replaceAll("^\\[|\\]$", "") + "\n" +
                "Введите интересующий процессор: ");
        ...
        if (pr.contains(oper)) {
            ...
            System.out.println();
            for (Notebook note : notebooks) {
                if (note.getProcc().equals(oper)) {
                    note.showInfo();
                }
            }
        } else {
            System.out.println("Нет такой ОС. Выберите другую.");
            filterProcc(notebooks, scanner);
    ...
    public static void filterColor(HashSet<Notebook> notebooks, Scanner scanner){
        ...
        if (colors.contains(oper)) {
            ...
            System.out.println("Нет такого цвета. Выберите другой вариант");
            filterColor(notebooks, scanner);
        }
    }
    // Функция вывода всего списка ноутбуков
